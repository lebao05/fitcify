...
const createPlaylist = async (req, res, next) => {
  try {
    const data = await playlistService.createPlaylist(req.user._id, req.body, req.file);
...
const addSongToPlaylist = async (req, res, next) => {
  try {
    ...
    const data = await playlistService.addSongToPlaylist(playlistId, req.user._id, songId);
...
const removeSongFromPlaylist = async (req, res, next) => {
  try {
    ...
    const data = await playlistService.removeSongFromPlaylist(playlistId, req.user._id, songId);
