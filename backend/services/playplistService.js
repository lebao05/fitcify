...
const updatePlaylistDetails = async (playlistId, ownerId, body, file) => {
  ...
  if (file) {
    // upload new cover
    const { secure_url, public_id } = await uploadToCloudinary(file.path, 'playlists');
    ...
    if (playlist.imageUrl) {
      ...
      if (oldId && oldId !== public_id) {
        ...
        catch (e) { console.warn('⚠️ Failed to destroy old cover:', oldId, e.message); }
      }
    }
...
const removeSongFromPlaylist = async (playlistId, ownerId, songId) => {
  ...
  const index = playlist.songs.findIndex(id => id.toString() === songId.toString());
  if (index === -1) {
...
const deletePlaylist = async (playlistId, ownerId) => {
  ...
  if (playlist.imageUrl) {
    ...
    if (oldId) {
      try { await cloudinary.uploader.destroy(oldId, { invalidate: true }); }
      catch (e) { console.warn('⚠️ Failed to destroy cover:', oldId, e.message); }
    }
  }
